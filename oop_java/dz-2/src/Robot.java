public class Robot implements Actn{
    private String name;
    private Integer run_lngth;
    private Integer jump_hgth;


    public Robot(String name, Integer run_lngth, Integer jump_hght) {
        this.name = name;
    ...
    public void jump(Wall wall) {
        int h = wall.getHght();
        if(h <= jump_hgth) {
            System.out.printf("Робот перепрыгнул %s%n", h);
        }
    }
    @Override
    public void run(Treadmill treadmill) {
        ...
        if(l <= run_lngth){
            System.out.printf("Робот пробежал %s%n", l);
        }
    }
