...
public class Cat implements Actn {
    private String name;
    private Integer run_lngth;
    private Integer jump_hgth;
    public Cat(String name, Integer run_lngth, Integer jump_hght) {
        this.name = name;
    ...
    public void jump(Wall wall) {
        int h = wall.getHght();
        if(h <= jump_hgth) {
            System.out.printf("Кот перепрыгнул %s%n", h);
        }
    }
    ...
    public void run(Treadmill treadmill) {
        ...
        if(l <= run_lngth){
            System.out.printf("Кот пробежал %s%n", l);
        }
    }
